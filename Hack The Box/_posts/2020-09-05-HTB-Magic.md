...
Magic Mahcine has very realistic vulerbailites, specially the ones that are related to web application. it requires thorough test and good enumeration.  
...
running strings against '/bin/sysinfo' and grep for above words with `-A1` option to get 1 line after each word.
