...
first thing comes to mind is file upload restriction issues, hence the name of the machine is "Magic" and a famous technique to prevent "malicious File Upload" in development word is called "Magic Number". The developer can implement "Magic Number" and chooses a specific type of file to be accepted and saved on the server. It called "Magic Number" becuase, it reads the first 4 bytes of the file. Matching it with file extenstion to determine whether to accept or refuse based on the estenstion and file signature that the developer hardcoded in the application.
